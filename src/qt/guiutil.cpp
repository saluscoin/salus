...
{
    // Convert salus:// to salus:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("salus://"))
    {
        uri.replace(0, 11, "salus:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "SaluS.lnk";
}
...
{
    return GetAutostartDir() / "salus.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("SaluS-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  salus-qt [" + tr("command-line options") + "]                     " + "\n";
